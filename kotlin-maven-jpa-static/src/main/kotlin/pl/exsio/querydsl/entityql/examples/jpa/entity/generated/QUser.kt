 ...
 */
 ...
 class QUser : QStaticModel<User<*>> {
     ...
     val typeStr: QEnumPath <UserTypeByName> = run {
         val config = QPathConfig(UserTypeByName::class.java, Enum::class.java, "TYPE_STR", false, 4, 12)
     ...
     val typeOrd: QEnumPath <UserTypeByOrdinal> = run {
         val config = QPathConfig(UserTypeByOrdinal::class.java, Enum::class.java, "TYPE_ORD", false, 5, 12)
     ...
     val order: ForeignKey<Order> = run {
         ...
         val order = this.createInvForeignKey<Order>(listOf(order0), listOf("USER_ID"))
         this.inverseJoinColumnsMap.put("order", order)
         order
      }

     val _primaryKey: PrimaryKey<User<*>> = run {
